/* =====================================================================
   FaqCms — step 2 of 2: schema + seed data.

   BEFORE running this: make sure your query window's active database
   is FaqCms (the dropdown in SSMS's toolbar, or run "USE FaqCms;" once
   by itself first). This file assumes it, on purpose, so it never gets
   run against master by mistake.

   Safe to re-run — every step is guarded by IF NOT EXISTS / IF OBJECT_ID
   checks, and by an __EFMigrationsHistory stamp per migration, so running
   it twice (or letting the app's own db.Database.Migrate() run afterward)
   won't try to recreate anything or throw "object already exists".

   NOTE: You normally do NOT need to run this by hand at all. Program.cs
   calls db.Database.Migrate() on startup, so the app creates the schema
   itself the first time it runs against an empty FaqCms database. Use
   this script only if you want the schema to exist before the first
   run, or the app's SQL login doesn't have rights to create tables and
   a DBA needs to run this part separately.
   ===================================================================== */
...
IF DB_NAME() <> 'FaqCms'
BEGIN
    RAISERROR('Wrong database context — select FaqCms as the active database before running this script.', 16, 1);
    RETURN;
END
GO
...
/* Seed data — each table guarded independently (not as one all-or-nothing
   block) so that if a previous run got partway through and left, say,
   Categories populated but Articles empty, re-running this script fills
   in exactly what's missing instead of skipping everything because
   Categories already had rows. */
IF EXISTS (SELECT 1 FROM sys.tables WHERE name = 'Categories') AND NOT EXISTS (SELECT 1 FROM [Categories])
BEGIN
...
IF EXISTS (SELECT 1 FROM sys.tables WHERE name = 'Tags') AND NOT EXISTS (SELECT 1 FROM [Tags])
BEGIN
...
IF EXISTS (SELECT 1 FROM sys.tables WHERE name = 'Articles') AND NOT EXISTS (SELECT 1 FROM [Articles])
...
IF EXISTS (SELECT 1 FROM sys.tables WHERE name = 'ArticleSteps') AND NOT EXISTS (SELECT 1 FROM [ArticleSteps])
   AND EXISTS (SELECT 1 FROM [Articles] WHERE [Id] = 1)
BEGIN
...
IF EXISTS (SELECT 1 FROM sys.tables WHERE name = 'ArticleTags') AND NOT EXISTS (SELECT 1 FROM [ArticleTags])
   AND EXISTS (SELECT 1 FROM [Articles] WHERE [Id] = 1) AND EXISTS (SELECT 1 FROM [Tags] WHERE [Id] = 1)
BEGIN
